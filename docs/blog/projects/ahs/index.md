...
My code attempted to extract and decode as much data as possible from an AHS file. Below are my findings so far about the file structure, gleaned both from trying to analyse the file itself and via an attempt to disassemble the code that creates the file on the server.
...
We can also try to see how iLO creates the .ahs file, and the files inside it (e.g. the .bb and .pkg files). To that end, we can have a sneaky peek inside the iLO firmware to see how HP does its thing.
